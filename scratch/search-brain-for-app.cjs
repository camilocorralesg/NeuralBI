...
function searchDir(dir) {
  ...
  list.forEach(file => {
    ...
    if (stat && stat.isDirectory()) {
      if (file !== '.system_generated' && file !== 'tempmediaStorage' && file !== 'browser_recordings') {
        try {
          searchDir(filePath);
        } catch(e) {}
      }
      ...
      if (file.endsWith('.js') || file.endsWith('.jsx') || file.endsWith('.txt') || file.endsWith('.md')) {
        try {
          ...
          if (content.includes('function Manifesto') && content.length > 5000) {
...
console.log("Searching brain directory recursively...");
searchDir(brainDir);
